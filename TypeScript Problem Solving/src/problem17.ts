...
const findMenuItemsByTag = (tag:string,menus: Menu[]): Menu[]=>{
    const filteredRestaurant:Menu[] = menus.filter(menu=>{
        if(tag === menu.tag){
            return menu;
        }
    });

    if(filteredRestaurant.length !== 0) return filteredRestaurant;
